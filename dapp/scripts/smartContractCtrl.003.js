...
app.controller('smartContractController', [
    ...
    function smartContractCtrl($rootScope,
        ...
        $log.debug('[smartContractCtrl.js] smartContractController ver. 001 started');
        // --- Alerts:
        ...
        if ($rootScope.web3 && $rootScope.web3.isConnected()) {
            ...
            // var web3Contract = $rootScope.web3.eth.contract(
            //     $scope.contract.contract.abi // abiArray
            // ).at(
            //     $scope.contract.address
            // );
            // $log.info('Web3 ContractInstance ' + web3Contract.address + ' :');
            // // $log.log(web3Contract.constructor.name); // 'h' - why? because of minified version?
            // $log.log(web3Contract); // 'h'
            // //
            ...
            var getShareholderArr = function () {
                $scope.contract.shareholder.call().then(
                    function (shareholder) {
                        $log.debug('shareholder array:');
                        $log.debug(shareholder);
                    }
            ...
            $scope.getEthBalanceOf = function (address) {
                ...
                $log.debug('[getEthBalanceOf()] currentAccount: ' + currentAccount);
            ...
            //         $log.debug('(' + i + ') ' + $rootScope.web3.eth.accounts[i]);
            //         // acc ETH
            ...
            $scope.getAllShareholdersArray = function () {
                $log.debug('getAllShareholdersArray() started');
                var shareholderId = 0;
                var address;
                for (var i = 0; i !== -1; i++) {
                    shareholderId = i;
                    $scope.contract.shareholder.call(shareholderId).then(function (shareholderAddress) {
                            $log.debug('[getAllShareholdersArray()] shareholderAddress: ' + shareholderAddress);
            ...
            }; // end of $scope.getAllShareholdersArray()
